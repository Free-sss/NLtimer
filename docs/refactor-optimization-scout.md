...
- 将 Route/Screen 层的 `collectAsState()` 全量迁移到 `collectAsStateWithLifecycle()`。
- 新增 `core:debugui`，把字段调试弹窗从 `feature:debug` 下沉到 core 层，移除业务 feature 对 `feature:debug` 的依赖。
- 新增 `AppTagChip` 和 `ExpandableGroupCard`，完成标签 chip 与 activity group card 的第一批复用。
...
- `feature:tag_management`、`feature:management_activities` 曾依赖 `feature:debug`，主要为了字段详情/JSON 调试 UI。当前已下沉到 `core:debugui`。
...
feature:tag_management -> core:designsystem, core:data, core:debugui
feature:management_activities -> core:designsystem, core:data, core:debugui
feature:debug -> core:designsystem, core:data, core:tools, feature:home, core:behaviorui
core:debugui -> core:data, core:designsystem
...
- `feature:* -> feature:debug` 是架构反模式。当前已移除 `tag_management` 与 `management_activities` 对 `feature:debug` 的依赖。
...
- 多个 Route 曾使用 `collectAsState()`：`HomeRoute`、`CategoriesRoute`、`ThemeSettingsScreen`、`TagManagementScreen`、`DialogConfigScreen`、`DataManagementScreen`、`ActivityManagementScreen`、`ToolConsoleScreen`。当前已统一迁移到 `collectAsStateWithLifecycle()`。
...
1. 已将 `FieldInfo`、`FieldDetailDialog`、`toJsonString` 等调试展示能力移入 `core:debugui`。
2. 已将 `feature:debug` 改为依赖 `core:debugui`。
3. 已从 `feature:tag_management` 和 `feature:management_activities` 删除 `implementation(projects.feature.debug)`。
4. 后续若要继续压缩 debug 聚合，应处理 `feature:debug -> feature:home`。
...
- 已将 `feature/home` 的 `TagChip(TagUiState)` 改为薄 wrapper。
- 已将 `feature/tag_management` 的 `TagChip(Tag)` 改为薄 wrapper。
- 已将颜色亮度判断放入 `core:designsystem`。
...
- 已新增 `ExpandableGroupCard` 并在 `feature/management_activities/.../GroupCard` 复用 header/elevation。
- 后续可将 `feature/management_activities/.../GroupCard` 改名 `ActivityGroupCard`，避免和 core `GroupCard` 命名冲突。
...
- `TagChip` 两个实现已合并为 core API + feature wrapper。
...
- 已移除业务 feature 对 `feature:debug` 的依赖。
- 已将全项目 Route 层迁移到 `collectAsStateWithLifecycle()`。
- 为非 Compose core 模块调整 convention plugin，避免 `core:tools` 被迫引入 Compose。

P1：组件复用化

- 已抽 `AppTagChip`、`ExpandableGroupCard`；后续继续抽 `SettingsSection`。
...
- 已完成：`buildSrc` name、match 测试 fixture、AutoMirrored icons、`collectAsStateWithLifecycle()` 批量迁移。
...
- 已新建 `core:debugui`。
- 已移动 `FieldInfo`、`FieldDetailDialog`、JSON field helpers。
- 已删除 `feature:tag_management`、`feature:management_activities` 对 `feature:debug` 的依赖。

### 第 3 批：组件复用

目标：降低 UI 重复，实现统一视觉和 API。

- 已在 `core:designsystem` 新增 `AppTagChip`。
- 已改造 home/tag management wrapper。
- 已新增 `ExpandableGroupCard`，并改造 activity management。
...
./gradlew.bat :feature:home:compileDebugKotlin :feature:settings:compileDebugKotlin :feature:categories:compileDebugKotlin :feature:tag_management:compileDebugKotlin :feature:management_activities:compileDebugKotlin :feature:debug:compileDebugKotlin
./gradlew.bat :core:debugui:compileDebugKotlin :feature:tag_management:compileDebugKotlin :feature:management_activities:compileDebugKotlin :feature:debug:compileDebugKotlin
./gradlew.bat :core:designsystem:compileDebugKotlin :core:behaviorui:compileDebugKotlin :feature:home:compileDebugKotlin :feature:tag_management:compileDebugKotlin
```
...
- 生命周期迁移相关 feature 编译成功。
- `core:debugui` 与 debug 解耦相关 feature 编译成功。
- `AppTagChip` / `ExpandableGroupCard` 相关模块编译成功。
