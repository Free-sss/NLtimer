...
> 版本：v1.1 | 日期：2026-04-27 | 状态：已审查修订，待实现
...
保存/删除操作后，主页数据通过以下机制保证即时刷新：

```
保存成功
  → updateBehaviorDetail() 调用 Repository 写入 Room
  → Room 数据变更触发 Flow 推送
  → HomeViewModel.init() 中订阅的 loadHomeBehaviors() Flow 收到新数据
  → _uiState.update { buildUiState(newBehaviors) } 重新构建 UI 状态
  → HomeScreen 重组，网格列表即时更新

删除成功
  → deleteBehaviorDetail() 调用 Repository.delete(id)
  → Room 删除操作 → Flow 推送 → 同上链路
```

**关键设计点**：

1. `loadHomeBehaviors()` 在 `init` 中通过 `stateIn(WhileSubscribed(5000))` 订阅 Room Flow，保存/删除后自动触发重新查询
2. 时间填充操作（fillLeftGap / fillRightGap）本质是修改 startTime/endTime，复用同一刷新链路
3. 保存过程中 `isSaving = true` 防止 Sheet 提前关闭，保存成功后设置 `isDetailSheetVisible = false` + `detailBehavior = null`
4. 不依赖隐式刷新，数据变更路径可追踪：`Room Write → Flow Emit → UiState Update → Compose Recompose`
...
| [当前] | 开始/结束各一个 | 时间 = `LocalDateTime.now()`（每次点击时实时获取，非打开弹窗时的快照） | 同上                                  |     |
...
> **dayStart / dayEnd 定义**：
>
> - `dayStart` = 当天 `00:00:00.000` 的 epochMillis
> - `dayEnd` = 次日 `00:00:00.000` 的 epochMillis（即当天 24:00，等于 `today.plusDays(1).atStartOfDay(...)`）
> - 填充操作的时间范围为 **当日 00:00 ≤ 时间 < 次日 00:00**
...
| 修改结束时间       |   ❌    | ⚠️ 用户编辑结束时间并保存后，自动切换为 COMPLETED（非只读即切换） |    ✅     |
| [上尾] 快捷按钮    | ❌ 隐藏 |                                ✅                                 |    ✅     |
| [当前](开始)       |   ✅    |                                ✅                                 |    ✅     |
| [当前](结束)       | ❌ 隐藏 |                           ✅ + 自动完成                           |    ✅     |
...
fun isOverlapping(aStart: Long, aEnd: Long, bStart: Long, bEnd: Long): Boolean {
    ...
    // 注意：边界相等（aEnd == bStart）视为相邻不重叠，允许无缝衔接
}
```

检测逻辑：

1. 保存时查询当天（含 ±1 天边界）所有 ACTIVE/COMPLETED 行为
2. 遍历除当前行为外的所有行为，执行 `isOverlapping` 判断
3. 若存在重叠，弹出确认弹窗，列出冲突行为名称和时间段
4. 用户选择：「强制保存」（覆盖写入）或「返回调整」（关闭弹窗让用户自行修改时间）
5. PENDING 状态的行为不参与重叠检测（无确定时间）
...
| 跨天时间处理             | 昨日/明日行为的空闲间隙计算                | **填充操作限定当日范围**：查询使用当天 `dayStart~dayEnd`，仅调整当天内的时间边界。跨日行为（如 23:30→次日 01:00）的填充逻辑留待 v2 版本处理 |

### 8.1 性能优化考量

| 优化点                   | 措施                                                                                                 |
| ------------------------ | ---------------------------------------------------------------------------------------------------- |
| 滚轮 LazyColumn 重组控制 | 使用 `items(count, key = { it })` 提供稳定 key，避免滚动时无意义重组                                 |
| 滚轮项渲染缓存           | 设置 `LazyColumn` 的 `beyondBoundsItemCount = 3`，预加载上下各3项，防止快速滚动时空白闪烁            |
| 时间选择器状态联动       | 开始时间变化时，结束时间滚轮的有效范围通过 `remember(derivedStateOf { ... })` 派生计算，避免全局重组 |
| 快捷按钮动画             | 滚轮滚动使用 `animateScrollToItem`（而非 `scrollToItem`），动画时长 200ms，防止 jank                 |
| 标签列表渲染             | `TagPicker` 已使用 `FlowRow` + key，无需额外优化                                                     |
| 详情数据加载             | `getBehaviorWithDetails()` 为 suspend 单次查询，不产生持续 Flow 订阅，避免资源浪费                   |
| DetailSheet 生命周期     | Sheet 关闭时设置 `detailBehavior = null`，释放快照数据占用的内存                                     |

### 8.2 AddBehaviorSheet 重构验证清单

从 AddBehaviorSheet 中提取可复用组件后，必须验证以下用例：

| #   | 验证项                                                          | 预期结果                                               |
| --- | --------------------------------------------------------------- | ------------------------------------------------------ |
| 1   | 打开 AddSheet → 选择活动 → 选择标签 → 设置时间 → 保存           | 行为成功添加，网格即时显示                             |
| 2   | 打开 AddSheet → ActivityPicker 为空 → 点击 "+ 添加"             | AddActivityDialog 正常弹出，添加后 ActivityPicker 刷新 |
| 3   | 打开 AddSheet → TagPicker 为空 → 点击 "+ 添加"                  | AddTagDialog 正常弹出，添加后 TagPicker 刷新           |
| 4   | 打开 AddSheet → 切换 BehaviorNature（PENDING/ACTIVE/COMPLETED） | 时间选择区域正确显示/隐藏                              |
| 5   | 打开 AddSheet → 选择 ACTIVE → 保存                              | 旧 ACTIVE 行为自动结束，新行为开始                     |
| 6   | 打开 AddSheet → 填写备注 → 保存                                 | 备注正确持久化                                         |
| 7   | 打开 AddSheet → 下滑关闭（不保存）                              | Sheet 关闭，无数据变更                                 |
| 8   | 打开 AddSheet → 快速旋转屏幕                                    | Sheet 状态保持，数据不丢失                             |
| 9   | AddSheet 和 DetailSheet 先后打开 → 关闭                         | 两个 Sheet 独立工作，互不干扰                          |
